...
public class AdminTakePatientByBalanceRangeCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws CommandException {
        ...
        String strFirstRange = request.getParameter(RequestParameter.FIRST_RANGE);
        String strSecondRange = request.getParameter(RequestParameter.SECOND_RANGE);
        if (strFirstRange.isBlank() && strSecondRange.isEmpty()){
            router.setPage(FAIL_PAGE);
            return router;
        }

        try {
            Long firstRange = Long.valueOf(strFirstRange);
            Long secondRange = Long.valueOf(strSecondRange);

                List<Patient> userList = patientService.findByMoneyAccount(firstRange,secondRange);
